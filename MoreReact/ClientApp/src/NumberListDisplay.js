...
class NumberListDisplay extends React.Component {
    ...
    generateLi = (num, idx) => {
        ...
        return <li className={className} key={idx}>{num}</li>
    }

    render() { 
        return ( 
            <ul>
               {/* {this.props.numbers.map((num, i) => this.generateLi(num, i) )} */}
               {this.props.numbers.map((num, i) => <li key={i} className={num > 100 ? 'text-danger' : ''} >{num}</li>)}
            </ul>
         );
    }
}
